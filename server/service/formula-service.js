...
class FormulaService {
  ...
  async updatedDateFormula(_etd, _delivery_channel, item) {
    try {
      if (_delivery_channel && _etd) {
        const delivery_channel = await DeliveryChannelSchema.findById(_delivery_channel).exec()

        const calculateDate = (baseDate, fetchedDate, valueToAdd) => {
          return baseDate
            ? addDayToDate(baseDate, fetchedDate)
            : addDayToDate(valueToAdd, fetchedDate)
        }

        const eta = setTime(dayjs(_etd).add(delivery_channel.eta, 'day'))

        const date_do = calculateDate(item.eta_update, delivery_channel.date_do, eta)

        const declaration_issue_date = calculateDate(
          item.date_do_update,
          delivery_channel.declaration_issue_date,
          date_do
        )

        const train_depart_date = calculateDate(
          item.declaration_issue_date_update,
          delivery_channel.train_depart_date,
          declaration_issue_date
        )

        const train_arrive_date = calculateDate(
          item.train_depart_date_update,
          delivery_channel.train_arrive_date,
          train_depart_date
        )

        const store_arrive_date = calculateDate(
  ...
  async dateFormula(_etd, _delivery_channel) {
    try {
      if (_delivery_channel && _etd) {
        ...
        //it is needed for keeping last not null date and pass it to next calc function
        let lastNotNull = _etd
  ...
  async updateFormulaDates(newDate, dateType, item, deliveryChannel) {
    try {
      const delivery_channel = await DeliveryChannelSchema.findById(deliveryChannel).exec()
      ...
      if (dateType === 1) {
        ...
        const store_arrive_date = this.calculateDates(
          train_arrive_date,
          delivery_channel.store_arrive_date
        )
        return {
          eta: returnOldDate(delivery_channel.eta, etaOld),
          date_do: returnOldDate(delivery_channel.date_do, dateDoOld),
          declaration_issue_date: returnOldDate(
            delivery_channel.declaration_issue_date,
            declarationIssueDateOld
          ),
