...
const { checkBoolean, checkDate, splitStrings, castToNum } = require('../utils/tableDataHandle')
...
const { nullDatesToEnd } = require('../utils/nullDatesToEnd')
...
class ItemService {
  ...
  async createItem(req) {
    try {
      ...
      const duplicatesOrder = await checkDuplicates(orderNumbers, 'order_number')

      if (duplicatesOrder.isDuplicate) {
        return errorReturn(`Повторяющийся № заказа: ${duplicatesOrder.duplicate.join(', ')}`)
      }
  ...
  async updateDocs(req) {
    try {
      ...
      const newArray = oldArray.map((doc) =>
        doc.order_number === req.body.is_docs.order_number ? req.body.is_docs : doc
      )

      await ItemSchema.updateOne({ _id: req.body.bidId }, { is_docs: newArray })
  ...
  async getKeyFilters(key_name, isHidden, has_filters) {
    try {
      const declarationNumbers = await ItemSchema.find({ hidden: isHidden }, key_name)
        .populate('delivery_channel', 'name')
        .populate('store', 'name')
        .populate('stock_place', 'name')
        .exec()
      ...
      const values = Array.isArray(valuesArrays[0]) ? [].concat(...valuesArrays) : valuesArrays

      const clearArr = this.removeDuplicates(
  ...
  async findItemsBySearch(query_string, search_filter, isHidden) {
    try {
      ...
      if (items.length === 0) {
        ...
        return items.filter((item) => item !== null && item.hidden === isHidden)
      }
    } catch (error) {
      console.log(error)
      return []
  ...
  async test(getItems, isHidden) {
    try {
      ...
      return result.filter((res) => res !== null && res.hidden === isHidden)
    } catch (error) {
      return []
    }
  }
  ...
  async getItemsFilter(query_keys, isHidden) {
    try {
      ...
      Object.keys(objectForQuery).forEach((key) => {
        ...
        if (objectForQuery[key] === 'null' && this.isArrayPole(key)) {
          ...
          if (Object.keys(query).length > 0) {
            const match = {}
            Object.keys(query).map((item) => (match[item] = query[item]))
            return (query = [
              {
                $match: {
                  ...match,
      ...
      await redisClient.set('filtered_bids', JSON.stringify(itemsToReturn))
    ...
    try {
      const result = dataForChanges.map(async (data) => {
        ...
        if (newDate === null) {
          ...
          } = await FormulaService.updateFormulaDates(newDate, dateType, item, delivery_channel)
    ...
    try {
      ...
      if (newDate === null) {
        ...
        const {
          ...
          store_arrive_date,
          store_arrive_date_update,
        } = await FormulaService.updateFormulaDates(newDate, dateType, item, deliveryChannel)
  ...
  async calculateDates(req) {
    try {
      ...
      if (req.body.date === null) {
        ...
        if (req.body.delivery_channel.length > 0) delivery_channel = req.body.delivery_channel
        else if (item.delivery_channel.length > 0) delivery_channel = item.delivery_channel
        else delivery_channel = ''

        const formulaRes = await FormulaService.dateFormula(etd, delivery_channel)
  ...
  async updateItem(req) {
    try {
      ...
      const duplicatesOrder = await checkDuplicatesArray(orderNumbers, 'order_number', _id)

      if (duplicatesOrder.isDuplicate) {
        return errorReturn(`Повторяющийся № заказа: ${duplicatesOrder.duplicates}`)
      }
      ...
      if (duplicateContainerNumber.isDuplicate) {
        return errorReturn(`Повторяющийся № контейнера: ${duplicateContainerNumber.duplicate}`)
      }
      ...
      if (duplicatesInside.isDuplicate) {
        return errorReturn(`Повторяющийся внутренний №: ${duplicatesInside.duplicates}`)
      }
      ...
      if (duplicatesProform.isDuplicate) {
        return errorReturn(`Повторяющийся № проформы: ${duplicatesProform.duplicates}`)
      }
      ...
      if (duplicatesDeclaration.isDuplicate) {
        return errorReturn(`Повторяющийся № декларации: ${duplicatesDeclaration.duplicates}`)
      }
      ...
      const simple = req.body.simple_product_name.map(async (simpleName, index) => {
      ...
      if (newDocs === null) {
        return {
          success: false,
          error:
            'Такой способ изменения № заказа приведет к потере документов для подачи, изменения НЕ сохранены',
        }
      }
  ...
  async uploadExcel(file) {
    try {
      ...
      const filteredContainers = containerNums.filter((num) => num !== null && num !== undefined)
      ...
      const itemsUpdate = filteredContainers.map(async (num, index) => {
        ...
        for (const col of Object.keys(BidColumns)) {
          ...
          if (isCol) {
            ...
            if (isDecl) {
              ...
              requestObject[datesColumns[col]] = checkDate(tableRow[col]) !== null
            } else {
        ...
        await ItemSchema.findOneAndUpdate({ container_number: num, hidden: false }, requestObject)
      })
    ...
    try {
      ...
      const items = json[0].map(async (item) => {
        const doc = new ItemSchema({
          ...
          delivery_channel: (
            await DeliveryChannelRepository.getDeliveryChannelByName(item['Канал поставки'])
          )?._id,
          ...
          declaration_issue_date_update: checkDate(item['Дата выпуска декларации']) !== null,

          train_depart_date_update: checkDate(item['Дата отправки по ЖД']) !== null,

          train_arrive_date_update: checkDate(item['Дата прибытия по ЖД']) !== null,

          store_arrive_date_update: checkDate(item['Дата прибытия на склад']) !== null,

          hidden: checkDate(item['Дата прибытия на склад']) !== null,
        })
