...
class ProductService {
  ...
  async createProductHand(product, itemId, simpleName) {
    try {
      ...
      await ItemSchema.findByIdAndUpdate(itemId, {
        product: [newProduct._id, ...beforeId.product],
        product_has_added: productsAdded,
      })
    } catch (e) {
      SendBotMessage(
  ...
  async updateProduct(item_id, file, simple_name, old_name) {
    try {
      await ProductSchema.updateMany({ item_id, simple_name: old_name }, { simple_name })

      const item = await ItemSchema.findById(item_id).exec()

      const oldProductHasAddedValue = item.product_has_added
        ? item.product_has_added[old_name]
        : false
  ...
  async getProduct(item_id, simple_name) {
    try {
      return await ProductSchema.find({ item_id, simple_name })
    } catch (error) {
      SendBotMessage(
        `${dayjs(new Date()).format('MMMM D, YYYY h:mm A')}\nGET PRODUCT ERROR:\n${error}`
      )
  ...
  async getProducts(products_id) {
    try {
      const findProducts = products_id.map(async (id) => await ProductSchema.findById(id))
