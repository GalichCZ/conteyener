...
class ItemController {
  async itemCreate(req, res) {
    ...
    if (!creator) return res.status(403).json({ message: 'NO CREATOR FOUND !' })
  ...
  async getItems(req, res) {
    ...
    if (filtersMap && filtersMap.length > 0) {
      ...
      const result = await ItemService.findItemsBySearch(query_string, search_filter, hidden)

      res.json({ items: result })
    } else {
      const { items, totalPages } = await ItemService.getItems(req.params.page, hidden)
  ...
  async getHiddenItems(req, res) {
    const { items, totalPages } = await ItemService.getHiddenItems(req.params.page)
  ...
  async updateFormulaDatesAfterUpload(req, res) {
    ...
    const { success, error } = await ItemService.updateFormulaDatesAfterUpload(dataForChanges)
  ...
  async updateFormulaDates(req, res) {
    ...
    const result = await ItemService.updateFormulaDates(_id, dateType, newDate, deliveryChannel)
    ...
    try {
      ...
      if (items.length === 0) {
        ...
        Promise.all(getItems).then((result) => {
          return res.json(result.filter((res) => res !== null && res.hidden === isHidden))
        })
      } else {
        res.json(items.filter((item) => item !== null && item.hidden === isHidden))
      }
    } catch (error) {
      console.log(error)
  ...
  async uploadExcel(req, res) {
    const { lastDatesMap, success, error } = await ItemService.uploadExcel(req.file.path)
